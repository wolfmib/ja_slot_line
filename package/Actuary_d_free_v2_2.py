...
#    free_game:  
            middle of col:  appear any W1 , goes to respin-mode.   
            the chance is around 4/149 * 3 =  0.0805
...
# 🎁: Calculate the Respin Event 
"""
   INFO:   Start from slot-04:  In freegame,  in the middle of col, appear W1 , Enter Respin-Mode
"""
try:
    import package.ja_slot_count_w1_in_specific_col as count_w1
except:
    import ja_slot_count_w1_in_specific_col as count_w1
...
# 🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺
try:
    import package.Actuary_respin_event_1_d_2_2 as fg_event_un
except:
    import Actuary_respin_event_1_d_2_2 as fg_event_un

try:
    import package.Actuary_respin_event_2_d_2_2 as fg_event_de
except:
    import Actuary_respin_event_2_d_2_2 as fg_event_de

try:
    import package.Actuary_respin_event_3_d_2_2 as fg_event_tr
except:
    import Actuary_respin_event_3_d_2_2 as fg_event_tr
...
class agent():
    ...
    def __init__(self,default_excelname="job_card_03_mg_v2_case_1.xlsx",show_log=False,show_debug=False):
        ...
        # Set Weight
        print("[🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺]: v2: [30,35,35]: ❌ ")
        print("[🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺]: v3: [12,44,44]: ✅ ")
        self.fg_events_weight_list = []

        for _ in range(3):
            tem_val = int(input("Type:\n"))
            self.fg_events_weight_list.append(tem_val)

        print(self.fg_events_weight_list)
        input("Check...... \n")

        
        self.heros_rtp_list = [0.000000000001,  0.000000000001,   0.000000000001 ]
        self.heros_cnt_list = [0, 0, 0]
        self.avg_heros_rtp = 0.000000000001


        # 🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺
        print("[🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺]: please input the excelname:  job_card_01_fg_case_4.xlsx, all fg_event will being apply")
        job_card_name = input("Press\n")


        print("🕵🏻‍♂️🕵🏻‍♂️ 🏹🏹  🍺🍺]:\n",job_card_name)
        input("Check...\n")

        print("Start 🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️🕵🏻‍♂️")
        self.hero_un_agent = fg_event_un.agent(default_excelname=job_card_name)


        print("Start 🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹🏹")
        self.hero_de_agent = fg_event_de.agent(default_excelname=job_card_name)


        print("Start 🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺🍺")
        self.hero_tr_agent = fg_event_tr.agent(default_excelname=job_card_name)
    ...
    def run(self,pytest_flag=False):
        ...
        # 🎁: Calculate the Respin Event 
        #  INFO:   Start from slot-04:  In freegame,  in the middle of col, appear W1 , Enter Respin-Mode
        obtenir_respin_w1_cnts = count_w1.run(curr_list_r,"W1",2)
        ...
        # Basic checking the w1 cnts, you shall see W1 cnts with 0 or 1, cause my fg_table setting is only appear W1 in single col each run.
        assert obtenir_respin_w1_cnts == 0 or obtenir_respin_w1_cnts == 1 or obtenir_respin_w1_cnts == 2 or obtenir_respin_w1_cnts == 3 


        # 表格不會有 W1 同時在第三軸 超過一個的
        assert obtenir_respin_w1_cnts == 0 or obtenir_respin_w1_cnts == 1

        # Respin 
        respin_gain = 0.0
        if obtenir_respin_w1_cnts == 3:
            self.W1_3_cnts += 1

            # 💰 get respin_gain later
            # Skip code
        elif obtenir_respin_w1_cnts == 2:
            self.W1_2_cnts += 1

            # 💰 get respin_gain later
            # Skip code

        elif obtenir_respin_w1_cnts == 1:
            self.W1_1_cnts += 1

            # 💰 Decide which hero: [30, 35, 35]
            pick_value = jason_random_generator_v1(1,100)
            if pick_value >=1 and pick_value <= self.fg_events_weight_list[0]:
                print("Hero1 with cnt:   %d"%self.heros_cnt_list[0])    
                # table_list
                curr_list_r = self.slot_table_agent.obtenir_mg_roll_table_par_natural_random(self.total_run_cnt)
                respin_gain, _list_r, _pd, _event_index = self.hero_un_agent.run(pytest_flag=False)

                self.heros_cnt_list[0] += 1

                # 💰
                self.heros_rtp_list[0] += respin_gain
                self.avg_heros_rtp     += respin_gain
        
            elif pick_value > self.fg_events_weight_list[0] and pick_value <= (self.fg_events_weight_list[0]+self.fg_events_weight_list[1]):
                print("Hero2 with cnt:   %d"%self.heros_cnt_list[1])    

                curr_list_r = self.slot_table_agent.obtenir_mg_roll_table_par_natural_random(self.total_run_cnt)
                respin_gain, _list_r, _pd, _event_index = self.hero_de_agent.run(pytest_flag=False)

                self.heros_cnt_list[1] += 1

                # 💰
                self.heros_rtp_list[1] += respin_gain
                self.avg_heros_rtp     += respin_gain


            elif pick_value > (self.fg_events_weight_list[0]+self.fg_events_weight_list[1]):
                print("Hero3 with cnt:   %d"%self.heros_cnt_list[2])    

                curr_list_r = self.slot_table_agent.obtenir_mg_roll_table_par_natural_random(self.total_run_cnt)
                respin_gain, _list_r, _pd, _event_index = self.hero_tr_agent.run(pytest_flag=False)

                self.heros_cnt_list[2] += 1

                # 💰
                self.heros_rtp_list[2] += respin_gain
                self.avg_heros_rtp     += respin_gain


        elif obtenir_respin_w1_cnts == 0:
            pass
        else:
            print("Error , unexpected the w1 cnts  ",obtenir_respin_w1_cnts)
        ...
        self.total_gain            += curr_gain + respin_gain
        self.curr_rtp               = self.total_gain / self.total_run_cnt
        self.X2                    += (curr_gain + respin_gain) ** 2
        if curr_gain + respin_gain > 0:
            self.hit_cnts += 1
            self.win_rate  = float(self.hit_cnts/self.total_run_cnt)


        if self.total_run_cnt == 1000:
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_01 rtp :       %.4f     "%(float(self.heros_rtp_list[0]/self.heros_cnt_list[0])))
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_02 rtp :       %.4f     "%(float(self.heros_rtp_list[1]/self.heros_cnt_list[1])))
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_03 rtp :       %.4f     "%(float(self.heros_rtp_list[2]/self.heros_cnt_list[2])))
            total_heros_cnts = self.heros_cnt_list[0] + self.heros_cnt_list[1] + self.heros_cnt_list[2] 
            print("[🕵🏻‍♂️ 🏹 🍺]:     avg_heros    :       %.4f     "%(float(self.avg_heros_rtp/total_heros_cnts)))



        if self.total_run_cnt == 2000:
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_01 rtp :       %.4f     "%(float(self.heros_rtp_list[0]/self.heros_cnt_list[0])))
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_02 rtp :       %.4f     "%(float(self.heros_rtp_list[1]/self.heros_cnt_list[1])))
            print("[🕵🏻‍♂️ 🏹 🍺]:      hero_03 rtp :       %.4f     "%(float(self.heros_rtp_list[2]/self.heros_cnt_list[2])))
            total_heros_cnts = self.heros_cnt_list[0] + self.heros_cnt_list[1] + self.heros_cnt_list[2] 
        ...
        if self.total_run_cnt % 2000 == 1 and self.total_run_cnt > 3000:
            ...
            # 🏦: version 2.1
            print("self.W1_1_cnts  =     %8d     with ratio    %.4f   "%(self.W1_1_cnts,float(self.W1_1_cnts/self.total_run_cnt)))
            print("self.W1_2_cnts  =     %8d     with ratio    %.4f   "%(self.W1_2_cnts,float(self.W1_2_cnts/self.total_run_cnt)))
            print("self.W1_3_cnts  =     %8d     with ratio    %.4f   "%(self.W1_3_cnts,float(self.W1_3_cnts/self.total_run_cnt)))
        ...
        # 🎁
        return_gain = curr_gain + respin_gain
        return return_gain , return_curr_list_r, self.main_pd ,respin_gain
...
if __name__ == "__main__":
    ...
    for index in range(actuary_set_runs):
        ...
        if set_pytest == True:
            curr_gain, curr_list_r, _tem_pd , _respin_gain = test_agent.run(pytest_flag=True)

        else:
            curr_gain, curr_list_r, _tem_pd , _respin_gain  = test_agent.run()
